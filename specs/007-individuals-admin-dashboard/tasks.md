...
- [X] T013 [US3] Added the existence pre-check to `deleteIndividual()` in `src/features/individuals/individualService.ts` exactly as specified (`.maybeSingle()` pre-check, `NOT_FOUND` throw before any delete). Per `contracts/individual-delete-everywhere.md`.
...
- [X] T018 Ran `npx tsc -b` (clean), `npx eslint .` (0 errors, 4 pre-existing warnings unrelated to this change), `npx vitest run` (92/92 tests pass across 13 files), `npm run build` (succeeds). `npx playwright test --list` confirms all 35 e2e tests (including 7 new ones) parse and register correctly; full `npx playwright test` execution was **not run** — this sandbox has no `.env`/live Supabase project (only `.env.example`), so e2e tests cannot authenticate, and the new migration (T001) was never applied to a real database. Recommend, before merging: (1) apply migration 0021 to a dev/staging Supabase project, (2) run `npm run test:e2e` against that project, paying particular attention to the multi-tree delete regression test (User Story 3), which is the one new test directly exercising the `enforce_last_tree_membership()` trigger fix.
